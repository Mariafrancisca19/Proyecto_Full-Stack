...
function EditarCitas({citas}) {
   ...
   useEffect(() =>{
     const obtenerCitasAgendadas = async()=>{
       const CitasAgendadas = await obtenerDatos('Agendar_Cita/')
  ...
  return (
    ...
    <h2 className="titulo-admin">Usuario: <span></span></h2>
    ...
    <h2 className="titulo-admin">Servicio: <span></span></h2>
    ...
    <h2 className="titulo-admin">Fecha: <span></span></h2>
    <input type='date' onChange={(e) => { setFecha(e.target.value)}}></input>

    <p className='titulo-admin'>Descripcion: </p>
